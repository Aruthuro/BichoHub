...
export async function getOcorrenciasAbertas(token: string): Promise<Ocorrencia[]> {
    const resp = await api.get('/coletores/ocorrencias/abertas', { headers: headers(token) });
    return resp.data;
}

export async function getOcorrenciaById(token: string, id: number): Promise<Ocorrencia> {
    const resp = await api.get(`/coletores/ocorrencias/${id}`, { headers: headers(token) });
    return resp.data;
}
...
export async function responderOcorrencia(token: string, id: number, resposta: string) {
    const resp = await api.patch(`/coletores/responder/${id}`, { resposta }, { headers: headers(token) });
    return resp.data;
}

export async function atualizarOcorrencia(token: string, id: number, dados: any) {
    const resp = await api.patch(`/coletores/ocorrencias/editar/${id}`, dados, { headers: headers(token) });
    return resp.data;
}

export async function finalizarOcorrencia(token: string, id: number, desfecho: string, descricao_soltura?: string) {
    const resp = await api.patch(`/coletores/ocorrencias/encerrar/${id}`, { desfecho, descricao_soltura }, { headers: headers(token) });
    return resp.data;
}
